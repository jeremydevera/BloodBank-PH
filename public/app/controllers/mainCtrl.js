...
.controller('mainCtrl', function(Auth, $timeout, $location, $rootScope, $window, $interval, User, AuthToken) {
    ...
    var showModal = function(option) {
        ...
        if (option === 1) {
            ...
            app.modalHeader = 'Logging Out, Please Wait'; // Set header
            $("#myModal").modal({ backdrop: "static" }); // Open modal
                ...
                $timeout(function() {
                    User.getPermission().then(function(data) {
                        if (data.data.permission === 'admin') {
                            app.authorized = true; // Set user's current permission to allow management
                            app.loadme = true; // Show main HTML now that data is obtained in AngularJS
                            $location.path('/superadmin_dashboard'); // Redirect to home
                        } else  if (data.data.permission === 'user' || data.data.permission === 'moderator') {
                            app.loadme = true; // Show main HTML now that data is obtained in AngularJS
                            $location.path('/coordinator_newsfeed'); // Redirect to home
                        }
                    });
...
.controller('criticalstatusCtrl', function(Bloodstock) {
    var app = this;
    ...
    // Function: get all the Newsfeeds from database
        ...
        Bloodstock.getBloodstock().then(function(data) {
            // Check if able to get data from database
            if (data.data.success) {
                // Check which permissions the logged in user has
                    app.bloodstocks = data.data.bloodstocks; // Assign users from database to variable                
